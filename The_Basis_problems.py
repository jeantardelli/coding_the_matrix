...
def is_independent(L):
    ...
    Example:
        ...
        True
    '''
    return False if any([is_superfluous(L,i) for i in range(len(L))]) else True
...
def subset_basis(T):
    ...
    '''
    B = list()
    [B.append(T[i]) for i in range(len(T)) if is_independent(B+[T[i]])]
    return B
...
def exchange(S, A, z):
    ...
    for i in range(len(B)):
        if is_superfluous(B, i) and B[i] not in A and B[i] != z:
            return B[i]
