...
def makeInverseIndex(strlist):
    ...
    True
    """
    wordset = {word: set() for wordlist in strlist for word in wordlist.split()}
    for index, wordlist in enumerate(strlist):
        for word in wordlist.split():
             if word in wordset:
                 wordset[word].add(index)
    return wordset
...
def andSearch(inverseIndex, query):
    ...
    """
    query  = set(query)
    search = set([i for sets in inverseIndex.values() for i in sets])
    for word, index in inverseIndex.items():
        if word in query:
            search = search & index
    return search