...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    return {(r,c): A[r,c] for r in A.D[0] for c in A.D[1]} == {(r,c): B[r,c] for r in B.D[0] for c in B.D[1]}
...
def matrix_matrix_mul(A, B):
    ...
    """
    assert A.D[1] == B.D[0]
    return Mat((A.D[0], B.D[1]), {(r,c): sum([A[r, k] * B[k, c] for k in A.D[1]]) for r in A.D[0] for c in B.D[1]})
