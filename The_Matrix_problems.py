...
def lin_comb_mat_vec_mult(M, v):
    ...
    '''
    assert(M.D[1] == v.D)
    return sum([v[x]*y for x,y in mat2coldict(M).items()])
...
def lin_comb_vec_mat_mult(v, M):
    ...
    '''
    assert(v.D == M.D[0])
    return sum([v[x]*y for x,y in mat2rowdict(M).items()])
...
def dot_product_mat_vec_mult(M, v):
    ...
    '''
    assert(M.D[1] == v.D)
    return Vec(set(mat2rowdict(M)), {row: u*v for row,u in mat2rowdict(M).items()})
...
def dot_product_vec_mat_mult(v, M):
    ...
    Examples:
      ...
      '''
    assert(v.D == M.D[0])
    return Vec(set(mat2coldict(M)), {col: v*u for col,u in mat2coldict(M).items()})
...
def Mv_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return coldict2mat({c: A*v for c,v in mat2coldict(B).items()})
...
def vM_mat_mat_mult(A, B):
    assert A.D[1] == B.D[0]
    return rowdict2mat({r: v * B for r,v in mat2rowdict(A).items()})
